...
class Sampler(Ddpm_base):
    ...
    def _simple_guided_sample_batch(
        ...
        Args:
            truth_sample_batch (torch.Tensor): Ground truth image batch for
            guidance. guidance_loss_scale (float): Scaling factor for the
            guidance loss between [0 - 100]. random_noise (bool):
            Whether to use random noise as the initial sample.
        Returns:
    ...
    def sample(self, filename_format="_sample_{i}.npy"):
        ...
        """
        if is_main_gpu():
            sampling_info = self.config.n_sample * (
                torch.cuda.device_count() if torch.cuda.is_available() else 1
            )
            self.logger.info(f"Sampling {sampling_info} images...")

        i = self.gpu_id if type(self.gpu_id) is int else 0
        if self.config.sampling_mode != "simple":
            dataloader_iter = iter(self.dataloader)  # type: ignore
        b = 0
        with tqdm(
            ...
            while b < self.config.n_sample:
                ...
                if self.config.sampling_mode == "simple":
                    ...
                    cond = next(dataloader_iter)["img"][:batch_size].to(
                        self.gpu_id
                    )
                    ...
                    raise ValueError(
                        f"Sampling mode {sampling_mode} not supported."
                    )
                for s in samples:
