...
def main_train(config):
    ...
    # Itération sur les données du dataloader
    for batch in train_data:
        # Ajouter les données du batch à la liste
        all_data.append(batch["img"])

    # Concaténation de tous les tensors en un seul tensor
    all_data = torch.cat(all_data)

    # Calcul des statistiques
    mean = all_data.mean().item()
    max_value = all_data.max().item()
    std_dev = all_data.std().item()

    # Affichage des statistiques
    print("Statistiques globales pour l'ensemble du jeu de données :")
